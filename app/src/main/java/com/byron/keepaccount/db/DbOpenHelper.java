...
public class DbOpenHelper extends SQLiteOpenHelper {
    ...
    public DbOpenHelper(@NonNull Context context){
        //数据库名为account.db , 当前版本为1
        super(context, Constant.ACCOUNT, null, 1);
    ...
    public void onCreate(SQLiteDatabase db) {
        //创建typebd表
        try{
            String createTypeDbSql = "create table " + Constant.TYPE_DB_NAME + "(id integer primary key autoincrement, typeName varchar(20), selectedImageId integer, selectImageId integer, kind integer)";
            db.execSQL(createTypeDbSql);
        }catch (Exception e){
            Log.d(TAG, "创建数据库失败： " + e.toString());
        }
        Log.d(TAG, "成功创建typeDb数据库");

        insertTypeData(db);

        //对应acountBean类
        try{
            String createAccountDb = "create table " + Constant.ACCOUNT_DB_NAME + "(id integer primary key autoincrement, typeName varchar(10), beiZhu varchar(20), time varchar(60)," +
                "year integer, month integer, day integer, kind integer, money float, imageId integer)";
            db.execSQL(createAccountDb);

        }catch (Exception e){
            e.printStackTrace();
        }
        Log.d(TAG, "成功创建accountDb数据库");


    }

    public void insertTypeData(SQLiteDatabase db){
        //往typeDb数据库里加入数据
        String sql = "insert into " + Constant.TYPE_DB_NAME + "(typeName, selectImageId, selectedImageId, kind) values(?,?,?,?)";
        //支出fragment
